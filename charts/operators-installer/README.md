...
| operators[].installPlanApproverRetries       | `10`          | No        | Number of InstallPlan approver retries. When `installPlanApproverWaitLoopInsideJob` is `false`, this controls the Kubernetes Job backoffLimit. When `true`, this controls the number of InstallPlan lookup attempts made inside the Job.
...
| operators[].installPlanApproverWaitLoopInsideJob | `false`   | No        | When true, the InstallPlan approver waits and retries for the InstallPlan inside the Job Pod instead of relying on Kubernetes Job retries. The number of attempts is controlled by `installPlanApproverRetries` and the overall Job runtime is limited by `installPlanApproverActiveDeadlineSeconds`.
| operators[].installPlanVerifierRetries       | `10`          | No        | Number of InstallPlan verifier retries. When `installPlanVerifierWaitLoopInsideJob` is `false`, this controls the Kubernetes Job backoffLimit. When `true`, this controls the number of InstallPlan lookup attempts made inside the Job.
...
| operators[].installPlanVerifierWaitLoopInsideJob | `false`   | No        | When true, the InstallPlan verifier waits and retries for the InstallPlan inside the Job Pod instead of relying on Kubernetes Job retries. The number of attempts is controlled by `installPlanVerifierRetries` and the overall Job runtime is limited by `installPlanVerifierActiveDeadlineSeconds`.
